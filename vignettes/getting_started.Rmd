...
```
```{r, eval=T, echo=T}
muo_data <- read_rds('~/projects/Pando/data/nepi_test_pre.rds')
muo_data
```
...
data('motifs')
data('motif2tf')
patterning_genes <- read_tsv('~/projects/Pando/data/patterning_genes.tsv')
```
```{r, eval=T, echo=T}
...
Our `Regions` object has not gotten a new slot containing the `Motif` object. This object stores a sparse peak x motif matrix with the matches and some other information.
...
The function `infer_grn()` has many more parameters to choose different models or tweek the association between peaks and genes, but we'll go into this in another vignette.
