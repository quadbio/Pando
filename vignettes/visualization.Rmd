...
```{r, eval=T, echo=T}
muo_data <- read_rds('~/projects/Pando/data/nepi_test.rds')
muo_data@grn
```

## QC and module metrics

As we've showed you in the "Getting started with Pando" vignette, module discovery step involves applying some filtering criteria on the goodnes-fo-fit of the fitted models. We can plot this with `plot_gof()`
...
This `tbl_graph` object works with most [igraph](https://igraph.org/), [`ggraph`](https://ggraph.data-imaginist.com/index.html) and [`ggraph`](https://tidygraph.data-imaginist.com/index.html) functions for easy graph manipulation and you can use it to make more customized plots:
